...
from robot_arm_interfaces.msg import PoseCommand 


ROBOT_CONFIG = MoveItConfigsBuilder(robot_name="my_arm", package_name="robot_arm_moveit_config")\
                                    .robot_description_semantic("config/my_arm.srdf", {"name": "my_arm"})\
                                    .to_dict()
...
class CommanderNode(Node):
    ...
    def got_to_joints_target(self, joints):
        if len(joints) != 6:
            return
    ...
    def joint_command_callback(self, msg: Float64MultiArray):
        self.got_to_joints_target(msg.data)

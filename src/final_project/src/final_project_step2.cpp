...
private:
        ...
        const rclcpp_action::GoalUUID &uuid, std::shared_ptr<const MoveTurtle::Goal> goal)
    {
        ...
        if (!server_activated_){
            RCLCPP_WARN(this->get_logger(), "Action server is not activated yet.");
        ...
        const std::shared_ptr<MoveTurtleGoalHandle> goal_handle)
    {
        ...
        while (rclcpp::ok()) {
            ...
            if (!server_activated_){
                ...
                result->message = "Aborted beacuse server was deactivated.";
                goal_handle->succeed(result);
                RCLCPP_WARN(this->get_logger(), "Action server deactivated.");
                return;
            }
